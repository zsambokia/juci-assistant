...
WHERE
p.surface = w.surface
AND g.glass_type = w.glass_type

AND (
    p.install_side = w.install_side
...
AND (
  ...
  OR (
    ...
    AND r.visible_light_reflection_ext > 20
  )
...
(heat_priority + privacy_priority + light_priority)

AS final_score

FROM filtered_products
CROSS JOIN wizard_input
